...
async def get_products(
    ...
    image_by_sku = {row[0]: f"{image_base_url}/{row[0]}?w=300&nobg=1&t={row[2] or ''}" for row in image_rows}
    image_by_name = {}
    for row in image_rows:
        if row[1]:
            image_by_name[row[1].upper()] = f"{image_base_url}/{row[0]}?w=300&nobg=1&t={row[2] or ''}"
