...
# $2 markup on all shipping rates
SHIPPING_MARKUP_DOLLARS = 2.00

# Default sender address (THD HQ — Spring Hill, FL)
...
# LeafLife sender address (Madison, WI)
...
# ─── PUBLIC endpoint: real-time USPS rate quotes for e-commerce checkout ───

# LeafLife keywords — must match the frontend isLeafLife() logic
_LEAFLIFE_KEYWORDS = ["everyday", "premium", "essential", "smalls", "snowcaps"]


class ShippingRateRequest(BaseModel):
    """Request body for fetching shipping rates at checkout."""
    street1: str
    street2: str = ""
    city: str
    state: str
...
@router.post("/rates")
async def get_shipping_rates(body: ShippingRateRequest):
    """Public endpoint — returns real-time USPS rates with $2 markup.

    If any product name matches a LeafLife keyword, the shipment originates
    from the LeafLife warehouse (Madison, WI).  Otherwise it ships from
    THD HQ (Spring Hill, FL).
    """
    # Determine origin based on cart contents
    has_leaflife = any(
        any(kw in name.lower() for kw in _LEAFLIFE_KEYWORDS)
        for name in body.product_names
    )
    from_address = LEAFLIFE_FROM_ADDRESS if has_leaflife else DEFAULT_FROM_ADDRESS
    ...
    # Default parcel dimensions for a typical order
    parcel = {
        "length": "10",
        "width": "8",
        "height": "4",
    ...
    async with httpx.AsyncClient(timeout=30.0) as client:
        resp = await client.post(
            f"{SHIPPO_API_URL}/shipments/", headers=headers, json=shipment_data
        )
        if resp.status_code not in (200, 201):
            print(f"[shippo] Rate quote failed: {resp.status_code} {resp.text}")
            raise HTTPException(
                status_code=502,
    ...
    for rate in rates:
        provider = rate.get("provider", "")
        if "usps" not in provider.lower():
            continue
        base_amount = float(rate.get("amount", "0"))
        marked_up = round(base_amount + SHIPPING_MARKUP_DOLLARS, 2)
        usps_rates.append({
            "id": rate["object_id"],
            "provider": provider,
            "service_level": rate.get("servicelevel", {}).get("name", ""),
            "amount": str(marked_up),
            "amount_cents": int(round(marked_up * 100)),
            "currency": rate.get("currency", "USD"),
            "estimated_days": rate.get("estimated_days"),
            "duration_terms": rate.get("duration_terms", ""),
        })
    ...
    return {
        "rates": usps_rates,
        "origin": "leaflife" if has_leaflife else "hq",
    }
